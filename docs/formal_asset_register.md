...
| A11 | Formula/Proof Archive Corpus | Text/PDF proof assets and formula documents under repo/docs | Medium | Medium-High | Whitepaper support, partner diligence, technical marketing collateral | Limited direct monetization without executable/provable tie-back -- A13 is the only asset here that supplies one, and only for the two claims listed against it | P3 |
...
| A13 | Machine-Checked Proof Layer (Lean 4) | `SutraWS/` (10,596 lines, 2,006 named theorems); built and axiom-audited by the `sutraws-lean` CI job; described in `docs/lean_formalisation.md` | High (as proof), Low (as coverage) | High | Diligence evidence, formal-methods positioning, certification narrative | Covers two things only: the alpha->0 identity-collapse guarantee for all 29 operators (`Contracts.identity_preserved`, replacing the kernel's `1e-10` float spot-check), and the triangular identity T(29)=435. The 9,940-line counter layer says nothing about the operator semantics -- 1,682 of its theorems are the 29x29 instances of `a+i+j = a+j+i`. Do not present it as a formalisation of the sutras | P1 |
